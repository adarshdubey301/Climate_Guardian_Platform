                ...
                with st.spinner("🔬 Analyzing with AI... Checking if food is compostable..."):
                    ...
                    try:
                        ...
                        # ✅ COMPOSTABLE FOOD - AWARD POINTS
                            ...
                            if groq_client:
                                prompt = f"""
                                Explain why this food is compostable in Hinglish (short & friendly):
                                Food: {result['food_name']}
                                Confidence: {result['confidence']:.0f}%
                                Category: {result['category']}
                                
                                Give me in Hinglish (50-100 words):
                                1. Why it's compostable
                                2. How to compost it
                                3. Environmental benefits
                                Keep it SHORT, conversational Hinglish, use emojis!
                                """
                                insights = get_groq_response(prompt, model="llama-3.1-8b-instant")
                                st.markdown(f'<div class="eco-card" style="background: #e3f2fd; border-left: 5px solid #4caf50;"><strong>🎙️ Hinglish Insights:</strong><br>{insights}</div>', unsafe_allow_html=True)
